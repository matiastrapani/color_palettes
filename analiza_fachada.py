...
def ejecutar_analisis_color(filename: str, dir_entrada: str | None = None, dir_mascara: str | None = None):
    ...
    estado_reducido = procesador.reducir_resolucion(
        estado_original, 
        ancho_objetivo=config.ancho_analisis
    )

    #estado_reducido = procesador.submuestrear_por_pasos(
    #    estado_original, 
    #    paso=config.paso_submuestreo  # O el valor que quieras usar de tu configuración
    #)
    ...
    estado_filtrado = agrupador.filtro_desplazamiento_media(estado_reducido, radio_color=config.radio_desplazamientos)
    ...
    paleta_mediana = agrupador.filtro_paleta_mediana(estado_filtrado, n_colores=config.n_paleta_mediana)
    ...
    visualizador = VisualizadorEspacioColor(config=config, metricas=metricas)
    ...
    visualizador.inicializar_grafico_3d(
        estado_analisis=estado_filtrado,  # <- Enviamos el objeto de estado completo
        estado_original=estado_original,
        anclas=anclas,
